...
@dataclass
class SplatfactoOnMeshDupModelConfig(ModelConfig):
    """Splatfacto Model Config, nerfstudio's implementation of Gaussian Splatting"""

    _target: Type = field(default_factory=lambda: SplatfactoOnMeshDupModel)
...
class SplatfactoOnMeshDupModel(Model):
    ...
    config: SplatfactoOnMeshDupModelConfig
    ...
    def populate_modules(self):
        ...
        self.faces_axis_x = horizon_vecs = horizon_vecs.cuda()
        self.faces_axis_y = horizon_cross_vecs = torch.cross(self.normals, horizon_vecs, dim=-1).reshape(-1, 3)
    ...
    def means(self):
        ...
        mesh_faces_verts = self.mesh_faces_verts[self.gaussians_to_mesh_indices]
        ...
        if self.config.unconstrained_elevate:
            ...
            maximum_elevate = radius.reshape(-1, 1) * 0.5
            minimum_elevate = -radius.reshape(-1, 1) * 0.5
        ...
        means = torch.sum(mesh_faces_verts * bary_coords.reshape(-1, 3, 1), dim=1).reshape(-1, 3)
        means += elevate
    ...
    def quats(self):
        ...
        quats = quaternion_multiply(faces_quats, base_quats)
    ...
    def split_gaussians(self, split_mask, samps):
        ...
        x_axis_splits = self.faces_axis_x[self.gaussians_to_mesh_indices][split_mask].repeat(samps, 1)
        y_axis_splits = self.faces_axis_y[self.gaussians_to_mesh_indices][split_mask].repeat(samps, 1)
        ...
        mesh_faces_verts_splits = mesh_faces_verts.repeat(samps, 1, 1)
        v1 = mesh_faces_verts_splits[:, 0]
        v2 = mesh_faces_verts_splits[:, 1]
        v3 = mesh_faces_verts_splits[:, 2]

        # Calculate u, u1, and u2 for the batch
        u = v_sample_proj_splits - v1  # (N, 3)
        u1 = v2 - v1  # (N, 3)
        u2 = v3 - v1  # (N, 3)

        # Stack u1 and u2 along the last dimension to form matrix U (N, 3, 2)
        U = torch.stack([u1, u2], dim=2)  # (N, 3, 2)

        # Reshape U and u to make sure they are in 2D format as required by torch.linalg.lstsq
        U_reshaped = U.reshape(-1, 3, 2)  # (N, 3, 2)
        u_reshaped = u.reshape(-1, 3, 1)  # (N, 3, 1)

        # Solve for lambda_2 and lambda_3 for the batch using torch.linalg.lstsq
        lambdas = torch.linalg.lstsq(U_reshaped, u_reshaped).solution.squeeze(-1)  # (N, 2)

        # Extract lambda_2 and lambda_3 for the batch
        lambda_2_batch = lambdas[:, 0]
        lambda_3_batch = lambdas[:, 1]

        # Calculate lambda_1 for the batch
        lambda_1_batch = 1 - lambda_2_batch - lambda_3_batch

        # Combine the lambdas for each vector in the batch
        lambdas = torch.stack([lambda_1_batch, lambda_2_batch, lambda_3_batch], dim=1)  # (N, 3)

        # err = torch.abs(torch.sum(lambdas.unsqueeze(-1) * mesh_faces_verts_splits, dim=1).reshape(-1, 3) - v_sample_proj_splits)
        # print("err: ", err.mean(), err.max(), err.min())
        # weird_lambdas_cnt = torch.sum(torch.logical_or(lambdas < 0, lambdas > 1).float())
        # print("weird_lambdas_cnt: ", weird_lambdas_cnt)
        #
        # assert False


        new_bary_coords = torch.log(lambdas)
        new_normal_elevates = self.gauss_params["normal_elevates"][split_mask].repeat(samps)
        ...
        new_quats = self.gauss_params["quats"][split_mask].repeat(samps)
        out = {
