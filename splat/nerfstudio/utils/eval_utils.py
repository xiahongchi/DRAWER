...
def resume_load_checkpoint(config: TrainerConfig, pipeline: Pipeline) -> Tuple[Path, int, Any]:
    ## TODO: ideally eventually want to get this to be the same as whatever is used to load train checkpoint too
    """Helper function to load checkpointed pipeline
...
def resume_setup_gs_uc(
    config_path: Path,
    eval_num_rays_per_chunk: Optional[int] = None,
    test_mode: Literal["test", "val", "inference"] = "test",
    update_config_callback: Optional[Callable[[TrainerConfig], TrainerConfig]] = None,
) -> Tuple[TrainerConfig, Pipeline, Path, int, Any]:
    """Shared setup for loading a saved pipeline for evaluation.
    ...
    # load checkpointed information
    checkpoint_path, step, loaded_state = resume_load_checkpoint(config, pipeline)

    param_groups = {
        name: [pipeline.model.gauss_params[name]]
    ...
    return config, pipeline, checkpoint_path, step, optimizers
