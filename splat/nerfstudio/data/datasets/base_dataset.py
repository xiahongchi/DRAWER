...
class InputDataset(Dataset):
    ...
    def get_numpy_image(self, image_idx: int) -> npt.NDArray[np.uint8]:
        ...
        # print("0 pil_image:", pil_image.size)
        if self.scale_factor != 1.0:
            width, height = pil_image.size
            # print("width, height:", width, height)
            newsize = (
                int(torch.floor(torch.tensor([width * self.scale_factor + 0.5]))),
                int(torch.floor(torch.tensor([height * self.scale_factor + 0.5])))
            )
            # print("newsize:", newsize)
            pil_image = pil_image.resize(newsize, resample=Image.Resampling.BILINEAR)
        # print("1 pil_image:", pil_image.size)
        image = np.array(pil_image, dtype="uint8")  # shape is (h, w) or (h, w, 3 or 4)
        # print("image np:", image.shape)
    ...
    def get_metadata(self, data: Dict) -> Dict:
        ...
        """
        additional_dict = {}
        if "mesh_depths" in self.metadata:
            mesh_depth = self.metadata["mesh_depths"][data["image_idx"]]
            additional_dict["mesh_depth"] = mesh_depth
