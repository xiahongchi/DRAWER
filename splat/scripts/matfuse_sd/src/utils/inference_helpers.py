...
def process_palette(palette_source, image_resolution, device):
    ...
    palette = pylette_extract_colors_mod(
        np.array(palette_source)[..., :3], palette_size=2, sort_mode="frequency"
    )
    ...
    # palette_colors = [np.clip(np.array(rgb).reshape(3) + (np.random.rand(3) - 0.5) * 200, 0, 255) for rgb in palette_colors]
    palette_colors = np.array(palette_colors).reshape(-1, 3)
    # palette_colors_jittered = np.clip(palette_colors + (np.random.rand(5, 3) - 0.5) * 40, 0, 255)
    palette_colors_jittered = np.clip(palette_colors, 0, 255)
    # print("palette_colors_jittered: ", palette_colors_jittered - palette_colors)
...
def run_generation(
    ...
    width_palette = 40
    height_palette = 20
    palette_save_np = np.repeat(np.repeat(np.clip(control["palette"].cpu().numpy() * 255.0, 0, 255).reshape(-1, 1, 3), width_palette, 1).reshape(-1, 1, width_palette, 3), height_palette, 1).reshape(-1, width_palette, 3).astype(np.uint8)
