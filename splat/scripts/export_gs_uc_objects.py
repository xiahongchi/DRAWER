...
def compose_for_export(means, shs_0, shs_rest, colors, opacities, scales, quats, prim_means_indices_prim_i):
    means_drawer_i = means.clone()[prim_means_indices_prim_i].cpu().numpy()
    shs_0_drawer_i = shs_0.clone()[prim_means_indices_prim_i].contiguous().cpu().numpy()
    shs_rest_drawer_i = shs_rest.clone()[prim_means_indices_prim_i].transpose(1, 2).cpu().numpy()
    colors_drawer_i = torch.clamp(colors.clone()[prim_means_indices_prim_i], 0.0, 1.0).data.cpu().numpy()
    opacities_drawer_i = opacities.clone()[prim_means_indices_prim_i].cpu().numpy()
    scales_drawer_i = scales.clone()[prim_means_indices_prim_i].cpu().numpy()
    quats_drawer_i = quats.clone()[prim_means_indices_prim_i].cpu().numpy()
...
# save_note_gs_internal_uc = "same_opt_50percent_50k_radius_half_mask_dilation_2_v4_fix_prismatic_v4_longer"
...
save_note_gs_internal_uc = "same_opt_50percent_v2"
...
object_gs_inidices = trace_dict["object_gs_inidices"]
...
object_gs_inidices = [torch.from_numpy(elem).cuda() for elem in object_gs_inidices]
...
for object_i, object_i_indices in enumerate(object_gs_inidices):
    map_to_tensors, count = compose_for_export(means, shs_0, shs_rest, colors, opacities, scales, quats, object_i_indices)
    remaining_indices[object_i_indices] = False

    export_path = os.path.join(save_dir, f"splat_object_{object_i:0>2d}.ply")
    print("export to ", export_path)
    write_ply(export_path, count, map_to_tensors)

    object_i_mesh_path = os.path.join(bakedsdf_dir, "separate/texture_mesh/", f"mesh-box_{object_i}_connected.obj")
    object_i_tex_mesh = load_objs_as_meshes([object_i_mesh_path], device="cuda")
    object_i_tex_mesh_trimesh = trimesh.Trimesh(
        object_i_tex_mesh.verts_packed().cpu().numpy(),
        object_i_tex_mesh.faces_packed().cpu().numpy(),
    )
    trimesh.exchange.export.export_mesh(object_i_tex_mesh_trimesh, os.path.join(collision_dir, f"object_{object_i}.ply"))


for drawer_i in tqdm(range(total_drawers_num)):

    interact_type = interact_type_list[drawer_i]
    # if drawer_i in [2, 6, 8]:
    if drawer_i in [8, 9, 14, 18]:
        ...
        collision_mesh_i_box_path = os.path.join(save_dir_boxes, f"box_{drawer_i:0>2d}.ply")
        collision_mesh_i_box = trimesh.exchange.load.load_mesh(collision_mesh_i_box_path)
        remaining_mesh = trimesh.util.concatenate([remaining_mesh, collision_mesh_i])


trimesh.exchange.export.export_mesh(remaining_mesh, os.path.join(collision_dir, f"main.ply"))

if True:
    export_every = 1500000
    main_idx = 0
    while torch.count_nonzero(remaining_indices) > export_every-1:
        print("0 remaining_indices: ", remaining_indices.shape, torch.count_nonzero(remaining_indices))
        select_indices = torch.argsort(means[remaining_indices, 0])[:export_every]
