...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image_indices", type=str, nargs='+', default=None, required=False
    )
    ...
    image_select_list = None
    if image_indices is not None:
        image_select_list = []
        for seq in image_indices:
            seq = seq.split("-")
            start = int(seq[0])
            end = int(seq[1])
            for idx in range(start, end+1):
                image_select_list.append(f"frame_{idx:0>5d}.png")


    with torch.no_grad():

        if image_select_list is not None:
            image_names = image_select_list
            print("image_select_list: ", image_select_list)
