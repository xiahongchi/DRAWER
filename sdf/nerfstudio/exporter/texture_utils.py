...
def export_textured_mesh(
    ...
    media.write_image(str(output_dir / f"{output_name}.png"), texture_image)
    ...
    lines_mtl = [
        ...
        "illum 2",
        "Ns 1.00000000",
        f"map_Kd {output_name}.png",
    ]
    lines_mtl = [line + "\n" for line in lines_mtl]
    file_mtl = open(output_dir / f"{output_name}.mtl", "w", encoding="utf-8")  # pylint: disable=consider-using-with
    file_mtl.writelines(lines_mtl)
    file_mtl.close()

    # create the .obj file
    lines_obj = ["# Generated with nerfstudio", f"mtllib {output_name}.mtl", f"usemtl {output_name}"]
    lines_obj = [line + "\n" for line in lines_obj]
    file_obj = open(output_dir / f"{output_name}.obj", "w", encoding="utf-8")  # pylint: disable=consider-using-with
    ...
    vertices = vertices_local_pad[:, :3] / vertices_local_pad[:, 3:]
    ...
    summary_log.append(f"OBJ file saved to {output_dir / f'{output_name}.obj'}")
    summary_log.append(f"MTL file saved to {output_dir / f'{output_name}.mtl'}")
    summary_log.append(
        f"Texture image saved to {output_dir / f'{output_name}.png'} "
