...
def collate_image_dataset_batch(batch: Dict, num_rays_per_batch: int, keep_full_image: bool = False):
    ...
    if "mask" in batch:
        ...
        patch_size = 128 # N
        assert num_rays_per_batch % patch_size == 0
        num_img_patch = num_rays_per_batch // patch_size # B
        img_indices = torch.floor(
            torch.rand((num_img_patch), device=device)
            * torch.tensor([num_images], device=device)
        ).long().reshape(-1, 1).expand(-1, patch_size)

        img_indices = img_indices.reshape(-1, 1)
        pixel_indices = torch.floor(
            torch.rand((num_rays_per_batch, 2), device=device)
            * torch.tensor([image_height, image_width], device=device)
        ).long()
        indices = torch.cat([img_indices, pixel_indices], dim=-1)
