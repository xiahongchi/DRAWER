...
def rasterize_silhouette_kaolin_pt3d(mesh_pt3d, render_camera, sigma):
    verts = mesh_pt3d.verts_packed()
    faces = mesh_pt3d.faces_packed()
    
    w2c = render_camera['w2c']
    camproj = render_camera['camproj']
    resolution = render_camera['resolution']
    
    soft_mask, face_idx_map, depth_map = rasterize_kaolin(verts, faces, w2c, camproj, resolution, sigma_inv=1 / sigma)
    valid = face_idx_map >= 0
    triangle_id = face_idx_map.long()
    
    return valid, triangle_id, soft_mask, depth_map
...
subprocess.run(f"rm -rf {ckpt_dir}/drawers/*", shell=True)
...
pose_dict_path = os.path.join(args.data_dir, "pose.pkl")
with open(pose_dict_path, 'rb') as f:
    pose_dict = pickle.load(f)

# print("pose_dict: ", sorted(list(pose_dict.keys())))
...
# mask and depth buffer
sam_masks = {}
depth_pmeshs = {}

# for soft rasterization
sigma = 1e-7
...
for drawer_img in drawers:
    for mask_id in drawers[drawer_img]:
        ...
        target_camera_info = pose_dict[stem]

        
        c2w = torch.from_numpy(target_camera_info["c2w"][:3, :4]).clone()
        c2w[:3, 1:3] *= -1
        ...
        c2w = torch.from_numpy(target_camera_info["c2w"]).cuda().clone()
        fx = target_camera_info["fx"]
        fy = target_camera_info["fy"]
        cx = target_camera_info["cx"]
        cy = target_camera_info["cy"]
        H, W = target_camera_info["H"], target_camera_info["W"]
        resolution = [H, W]
        
        n = 0.001
        f = 100  # infinite
        
        camproj = get_camera_perspective_projection_matrix(fx, fy, cx, cy, H, W, n, f)
        camproj = torch.from_numpy(camproj).float().to(device)
        ...
        for stem in tqdm(camera_stems):
            ...
            # w2c_list.append(w2c)
            # camproj_list.append(camproj)
            
            render_cameras.append({
                "w2c": w2c,
                "camproj": camproj,
            ...
            if stem not in sam_masks:
                sam_masks[stem] = -torch.ones((H, W), device="cpu").int()
            
            sam_masks[stem][sam_mask] = drawer_cnt
        ...
        with torch.no_grad():
            for stem_i in tqdm(range(len(camera_stems))):
                ...
                try:
                    _, _, _, depth_pmesh = rasterize_silhouette_kaolin_pt3d(drawer_mesh_pt3d, render_cameras[stem_i], sigma)
                except:
                    depth_pmesh = -torch.ones((H, W), device="cpu")
                
                depth_pmesh = depth_pmesh.cpu()
                
                if camera_stems[stem_i] not in depth_pmeshs:
                    depth_pmeshs[camera_stems[stem_i]] = torch.zeros((H, W), device="cpu")
                depth_pmeshs[camera_stems[stem_i]][sam_masks[camera_stems[stem_i]] == drawer_cnt] = \
                    depth_pmesh[sam_masks[camera_stems[stem_i]] == drawer_cnt]
        #         depth_pmesh_list.append(depth_pmesh.cpu())
...
Niter = int(600 * num_total_drawer)
...
for iter in opt_loop:
    ...
    camera_stems = opt_dict["camera_stems"][drawer_i]
    n_views = min(len(camera_stems), 20)
    ...
    # stems = np.arange(len(camera_stems))
    ...
    for stem_i in stems.tolist():
        ...
        try:
            _, _, silhouette, depth_map = rasterize_silhouette_kaolin_pt3d(box_primitive_p3d_transformed, render_cameras[stem_i], sigma)
        except:
            continue
        ...
        sam_mask = (sam_masks[camera_stems[stem_i]].detach().clone() == drawer_i).to(device)
        
        depth_pmesh = depth_pmeshs[camera_stems[stem_i]].detach().clone().to(device)
        depth_pmesh[torch.logical_not(sam_mask)] = -1.0
        ...
        if False:
            ...
            os.makedirs(f"{ckpt_dir}/drawers/logs/silhouette", exist_ok=True)
            
            silhouette_vis = silhouette.detach().cpu().numpy()
            silhouette_vis = silhouette_vis.reshape(H, W)
            Image.fromarray((silhouette_vis * 255).astype(np.uint8)).save(
                os.path.join(f"{ckpt_dir}/drawers/logs/silhouette", f"{drawer_i}_{stem_i}_silhouette.png"))
            
            # visualize the sam mask
            sam_mask_vis = sam_mask.detach().cpu().numpy()
            sam_mask_vis = sam_mask_vis.reshape(H, W)
            Image.fromarray((sam_mask_vis * 255).astype(np.uint8)).save(
                os.path.join(f"{ckpt_dir}/drawers/logs/silhouette", f"{drawer_i}_{stem_i}_sam_mask.png"))
            
            # visualize the depth
            far = float(depth_pmesh.max() * 1.2)
            near = 0
            depth_vis = (depth.detach().cpu().numpy() - near) / (far - near)
            depth_vis = np.clip(depth_vis, 0, 1)
            depth_vis = (depth_vis * 255).astype(np.uint8)
            Image.fromarray(depth_vis).save(
                os.path.join(f"{ckpt_dir}/drawers/logs/silhouette", f"{drawer_i}_{stem_i}_depth.png"))
            
            # visualize the depth_pmesh
            depth_pmesh_vis = (depth_pmesh.detach().cpu().numpy() - near) / (far - near)
            depth_pmesh_vis = np.clip(depth_pmesh_vis, 0, 1)
            depth_pmesh_vis = (depth_pmesh_vis * 255).astype(np.uint8)
            Image.fromarray(depth_pmesh_vis).save(
                os.path.join(f"{ckpt_dir}/drawers/logs/silhouette", f"{drawer_i}_{stem_i}_depth_pmesh.png"))
    ...
    if global_opt:
        ...
        for drawer_glb_opt_i in range(num_total_drawer):
            ...
            sampled_pts = sample_points_from_meshes(box_primitive_p3d_glb_opt, num_samples=5000).detach().reshape(-1, 3)
    ...
    if global_opt:
        loss = 5. * loss_mask + 5. * loss_depth + 10 * loss_glb + 30 * loss_chamfer
    else:
...
with torch.no_grad():
    for drawer_i in range(num_total_drawer):
        ...
        c2w = torch.from_numpy(target_camera_info["c2w"]).cpu().clone()
        c2w[:3, 1:3] *= -1
        w2c = torch.inverse(torch.cat([c2w, torch.from_numpy(
            np.array([0, 0, 0, 1]).reshape(1, 4)
        ).to(c2w.device)], dim=0)).numpy()
